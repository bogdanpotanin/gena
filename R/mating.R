#' Mating
#' @description Mating method (algorithm) to be used in the
#' genetic algorithm.
...
#' @param n_parents even positive integer representing the number of parents.
...
#' \eqn{n_{\text{population}}} equals to \code{nrow(population)}.
#' 
...
#' there are \eqn{n_\text{parents}} (i.e. \code{n_parents}) parents selected.
...
#' produce a child, where \eqn{i} is odd.
...
#' identical chromosomes in population.
#' 
...
#' \deqn{p_{i}=\frac{w_{i}}{\sum\limits_{j=1}^{n_{\text{population}}}w_{j}}}
...
#' \deqn{w_{i}=1 \implies p_{i}=\frac{1}{n_{\text{population}}}}
#' 
...
#' \eqn{r_{j}=n_{\text{population}}} and  \eqn{r_{k}=1}. The relationship
#' between weight \eqn{w_{i}} and rank \eqn{r_{i}} is as follows:
#' \deqn{w_{i}=\left(\frac{r_{i}}{n_{\text{population}}}\right)^{\tau_{1}}}
...
#' \deqn{w_{i}=\left(f_{i} - 
#'                   \min\left(f_{1},...,f_{n_{\text{population}}}\right) + 
#'                   \tau_{1}\right)^{\tau_{2}}}
...
#' By default \code{par = min(5, ceiling(n_parents * 0.1))}.
#' 
#' Validation and default values assignment for \code{par} is performed inside
#' \code{\link[gena]{gena}} function not in \code{\link[gena]{mating.gena}}.
...
#' rows of this matrix equals to \code{n_parents} while the number of columns
...
{      
  ...
  n_pop <- nrow(population)                                # the number of chromosomes
  ind_pop <- 1:n_pop                                       # chromosomes indexes
  
  # Estimate the probability of being a parent
  # for each chromosome
  
  probs <- rep(1 / n_pop, n_pop)                           # default probabilities vector
  
  if (method == "rank")                                    # rank based probabilities
  {
    fitness_rank <- rank(fitness)                          # rank by fitness
    fitness_weight <- (fitness_rank / n_pop) ^ par[1]      # weight according to rank
  ...
  parents_ind <- rep(NA, n_parents)                        # vector to store
                                                           # parents indexes
  ...
  if (!self)
  {
    odd_ind <- seq(from = 1, to = n_parents, by = 2)
    if (method %in% c("constant", "rank", "fitness"))
    {
      for (i in odd_ind)
      {
  ...
  parents <- population[parents_ind, ]                     # parents and
...
# on the "method"
gena.mating.validate <- function(method, par, n_parents)
{
  # Validate the "method"
  ...
  if (method == "tournament")
  {
    if (!is.null(par))
    {
      if ((length(par) != 1) | (par < 1) | (par > n_parents) | 
         (!is.numeric(par)))
      {
        stop(paste0("Incorrect mating.par agrument. Please, insure that ",
                    "(length(mating.par) == 1), (mating.par >= 1)",
                    "(mating.par < (n_pop - n_elite)), and ",
                    "is.numeric(mating.par)",
                    "\n"))
      }
    } else {
      par <- min(5, ceiling(n_parents * 0.1))
    }
  }
