                            ...
                            fn.args = list(), gr.args = list())
{
  ...
  for (i in 1:maxiter)
  {
    ...
    if (armajilo)
    {
      ...
      curvature <- abs(gr.div.new) <= (c2 * abs(gr.dir))
      if (curvature)
      {
        if (is_decrease)
        {
                       ...
                       ...)                                   # additional arguments for fn and gr
{
  ...
  for (i in 1:maxiter)
  {
    ...
    # are satisfied
    reltol.cond <- all(abs((x[i, ] - x[i + 1, ]) /
                            x[i, ]) < reltol)
    maxiter.cond <- i == maxiter
    if (reltol.cond | maxiter.cond)
    {
      iter <- i
      break
    }
...
# }
# 
# gr <- function(par, a = 20, b = 0.2)
# {
#   par <- par - 1
#   x <- par[1]
#   y <- par[2]
#   val <- c(NA, NA)
#   val[1] <- -0.707107 * a * b * x * exp(-0.707107 * b *sqrt(x ^ 2 + y ^ 2)) / 
#             sqrt(x ^ 2 + y ^ 2) -
#             pi * sin(2 * pi * x) * 
#             exp(0.5 * (cos(2 * pi * x) + cos(2 * pi * y)))
#   x <- par[2]
#   y <- par[1]
#   val[2] <- -0.707107 * a * b * x * exp(-0.707107 * b *sqrt(x ^ 2 + y ^ 2)) / 
#             sqrt(x ^ 2 + y ^ 2) -
#             pi * sin(2 * pi * x) * 
#             exp(0.5 * (cos(2 * pi * x) + cos(2 * pi * y)))
#   return(-val)
...
# 
# x0 <- c(5, 3)
# opt <- gena.local(x0, fn = fn, gr = gr)
# opt$solution
# opt$fn_value
# 
# opt2 <- optim(x0, fn = fn, gr = gr,
#               method = "BFGS", 
...
# 
# opt$hessian
# gena.hessian(gr = gr, par = opt$solution)
